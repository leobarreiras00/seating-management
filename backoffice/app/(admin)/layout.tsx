...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // Identidade Básica (do JWT)
  const [userInfo, setUserInfo] = useState<{username: string, role: string} | null>(null);
  // Identidade Completa (da API, para termos o ID e o Avatar)
  const [currentUser, setCurrentUser] = useState<any>(null);
  
  // Estados dos Menus e Modais
  const [showProfileMenu, setShowProfileMenu] = useState(false);
  const [showNotifMenu, setShowNotifMenu] = useState(false);
  const [showMyAccountModal, setShowMyAccountModal] = useState(false);

  // Estados para a Mudança de Password / Avatar
  ...
  useEffect(() => {
    ...
    client.on("connect", () => {
      // Subscrevemos aos tópicos de alertas operacionais e de segurança
    ...
    client.on("message", (topic, message) => {
      try {
        ...
        // Tenta interpretar a mensagem como JSON (se vier formatada), senão usa como texto
        let data: any = { message: payloadStr, title: "Novo Alerta de Sistema" };
        try { data = JSON.parse(payloadStr); } catch (e) {}

        // Define a severidade baseada no tópico
        ...
        if (data.type) type = data.type; // Se o JSON enviar o tipo, sobrepõe
        ...
        // Mantém apenas as últimas 50 notificações na memória
  ...
  // Vai buscar a foto e o ID real do utilizador ativo
  ...
  // Função auxiliar para os ícones das notificações
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            ...
            <div className="relative">
              <button 
                ...
                className={`p-2 rounded-xl transition-all relative ${showNotifMenu ? 'bg-purple-50 text-purple-600' : 'text-slate-400 hover:text-purple-600 hover:bg-slate-50'}`}
              >
              ...
              {showNotifMenu && (
                <div className="absolute right-0 mt-3 w-80 bg-white rounded-[1.5rem] shadow-[0_10px_40px_rgb(0,0,0,0.1)] border border-slate-100 p-4 z-50 animate-in slide-in-from-top-2">
                  <div className="flex justify-between items-center mb-4 px-1">
                    ...
                    {unreadCount > 0 && (
                      <button onClick={markAllAsRead} className="text-[10px] font-bold text-purple-600 hover:text-purple-800 transition-colors">
                        Marcar lidas
                      </button>
                    )}
                  </div>
            ...
            <div className="relative">
              ...
              >
                ...
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center border shadow-sm transition-transform group-hover:scale-105 overflow-hidden ${
                  userInfo?.role === 'SuperAdmin' ? 'bg-red-50 text-red-600 border-red-100' : 'bg-blue-50 text-blue-600 border-blue-100'
                }`}>
                  {/* Usa a foto real se existir, ou o ícone/imagem default */}
              ...
              {showProfileMenu && (
                <div className="absolute right-0 mt-3 w-56 bg-white rounded-[1.5rem] shadow-[0_10px_40px_rgb(0,0,0,0.1)] border border-slate-100 p-2 z-50 animate-in slide-in-from-top-2">
                  ...
                  {/* NOVO: A Minha Conta */}
                  <button 
                    onClick={() => { setShowMyAccountModal(true); setShowProfileMenu(false); }} 
                    className="w-full text-left px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 hover:text-purple-600 rounded-xl flex items-center gap-3 font-bold transition-colors mb-1"
                  >
                  ...
                  <button 
                    ...
                    className="w-full text-left px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 rounded-xl flex items-center gap-3 font-bold transition-colors"
                  >
      ...
      {showMyAccountModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md">
          <div className="bg-white rounded-[2.5rem] p-8 w-full max-w-lg shadow-2xl animate-in zoom-in-95 relative border border-white/50 flex flex-col md:flex-row gap-8">
            <button onClick={() => setShowMyAccountModal(false)} className="absolute top-6 right-6 p-2 text-slate-400 hover:text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors z-10">
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="md:w-2/3 border-t md:border-t-0 md:border-l border-slate-100 pt-6 md:pt-0 md:pl-8">
              ...
              {successMessage ? (
                <div className="flex flex-col items-center justify-center py-10 text-emerald-500 animate-in fade-in">
                ...
                <div className="space-y-4">
                  <input 
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-bold text-slate-900 focus:ring-2 focus:ring-purple-500 outline-none text-sm"
                  />
                  <input 
                    type="password" 
                  ...
                  <input 
                    ...
                    className={`w-full bg-slate-50 border rounded-xl px-4 py-3 font-bold text-slate-900 outline-none text-sm transition-all ${
                  ...
                  <button 
                    ...
                    disabled={isProcessing || !oldPassword || newPassword.length < 4 || newPassword !== confirmPassword}
                    className="w-full py-3 mt-2 rounded-xl font-bold text-white bg-slate-900 hover:bg-purple-600 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
                  >
